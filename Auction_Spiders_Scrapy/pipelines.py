...
class AuctionSpiderScrapyALiMySQLPipeline(object):
    def __init__(self):
        self.connect = pymysql.connect(host='localhost', user='root', password='chensy123', db='auction_spider_ali', port=3306)
        self.cursor = self.connect.cursor()
    ...
    def process_item(self, item, spider):
        try:
            ...
            sql = self.generate_sql('Auctions', item)
            self.cursor.execute(sql, tuple(item.values()))
            self.connect.commit()
            return item
        except pymysql.Error as e:
